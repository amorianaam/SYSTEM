          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {vipQueue.map((item) => (
              <PatientExaminationCard
                key={item.visitId}
                patient={{ ...item, is_exempt: item.is_exempt ?? 1 }}
                onClick={() => handleSelectPatient(item)}
                isVIP={true}
                actionText="معاينة المريض"
                showDate={true}
                dateLabel="وقت الزيارة:"
                dateValue={new Date(item.created_at).toLocaleTimeString("ar-LY", { hour: '2-digit', minute: '2-digit' })}
              />
            ))}
          </div>
        ) : (
