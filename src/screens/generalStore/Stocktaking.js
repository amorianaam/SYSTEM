...
import { ClipboardCheck, Plus, Check, Clock, Package } from 'lucide-react';
...
const Stocktaking = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (!window.confirm('اعتماد الجرد سيؤدي إلى تحديث الأرصدة الفعلية في المخزن وتسجيل الفروقات، هل أنت متأكد؟')) return;

    setSaving(true);
    try {
      const res = await fetch('/api/inventory/general/stocktaking', {
        method: 'POST',
    ...
    return (
      <div dir="rtl">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">جلسة جرد جديدة</h1>
  ...
  return (
    <div dir="rtl">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">الجرد الدوري</h1>
          <p className="text-gray-500 mt-1">سجل جلسات الجرد السابقة ومطابقة المخزون</p>
        </div>
        <button onClick={handleStartStocktaking} className="btn-primary flex items-center gap-2">
            ...
            {sessions.length === 0 ? (
              ...
              sessions.map(sess => (
                <tr key={sess.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 font-bold text-gray-800">STK-{sess.id.toString().padStart(4, '0')}</td>
